 ...
 * File: \client.go                                                            *
 * Project: simplerpc                                                          *
 * Created At: Tuesday, 2022/06/14 , 18:08:09                                  *
 * Author: elchn                                                               *
 * -----                                                                       *
 * Last Modified: Tuesday, 2022/06/14 , 20:57:25                               *
 * Modified By: elchn                                                          *
 * -----                                                                       *
 * HISTORY:                                                                    *
 * Date      	By	Comments                                                   *
...
// done 当调用结束时，会调用call.done()通知调用方。
func (call *Call) done() {
	call.Done <- call
}
...
func (client *Client) receive() {
	...
	for err == nil {
		...
		case h.Error != "": // call 存在，但服务端处理出错，即h.Error不为空。
			call.Error = fmt.Errorf(h.Error)
			err = client.cc.ReadBody(nil)
			call.done()
		default: // call存在，服务端处理正常，那么需要从body中读取Reply的值。
