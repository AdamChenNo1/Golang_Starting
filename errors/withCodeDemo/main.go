...
func main() {
	if err := bindUser(); err != nil {
		...
		// %#-v: Output full error stack details, useful for troubleshooting with JSON formatted output.
		fmt.Println("=========================> '%'#-v <==========================")
		fmt.Printf("%#-v\n\n", err)
		...
		if errors.IsCode(err, code.ErrEncodingFailed) {
			fmt.Println("this is an ErrEncodingFailed error")
		}

		if errors.IsCode(err, code.ErrDatabase) {
			fmt.Println("this is an ErrDatabase error")
		}
...
func queryDatabase() error {
	...
	return errors.WithCode(code.ErrDatabase, "user 'Linfei Kong' not found")
}
